            ...
            {pedidos.slice(0, 5).map(p => (
              <div key={p.id} className="flex items-center justify-between p-6 bg-slate-50 rounded-2xl border-2 border-slate-100 hover:border-blue-400 hover:bg-white transition-all group">
                ...
                <div className="text-right">
                  ...
                  <p className={`text-[8px] font-black uppercase tracking-[0.2em] px-3 py-1 rounded-full border-2 mt-2 inline-block ${p.status_importacao === 'COMPLETA' ? 'bg-emerald-100 text-emerald-950 border-emerald-300' : 'bg-amber-100 text-amber-950 border-amber-300'
                    }`}>
                    {p.status_importacao}
                  </p>
                </div>
